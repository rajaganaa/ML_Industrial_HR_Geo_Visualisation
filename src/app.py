...
# Using relative path to data folder assuming app is run from project root or src
# If running from root: data/
# If running from src: ../data/
...
if merged_df.empty:
    st.warning("Please place your CSV data files in the 'data' folder.")
else:
    # -----------------------------------------------------------------------------------------------------------------------------------------------------#
    # Separate state and district names
    # Check if 'India/States' column exists
    if 'India/States' in merged_df.columns:
        merged_df[['STATE', 'District']] = merged_df['India/States'].str.split(' - ', expand=True)
    else:
        # Fallback or skip if column missing
        pass
...
if selected == "Home":
    ...
    image_path = "img_102705_indian_economy.jpg"
    ...
    if merged_df.empty:
        st.error("No data available to display overview.")
    else:
        # Dataset
        ...
        X_tfidf = tfidf_vectorizer.fit_transform(merged_df['NIC Name'].astype(str))
        ...
        text_for_cluster = merged_df[merged_df['Cluster'] == selected_cluster]['NIC Name'].astype(str)
        ...
        if selected_work_type in merged_df.columns:
            ...
            st.warning(f"Column {selected_work_type} not found in data.")
    ...
    if merged_df.empty:
        ...
        # Tokenize and clean text data
        text = ' '.join(merged_df['NIC Name'].astype(str))
        ...
        # Scatter Plot
        if column_total in merged_df.columns and column_rural in merged_df.columns:
            fig1 = px.scatter(merged_df, x=column_total, y=column_rural, color='Cluster',
                              title=f'{worker_type} - Total vs Rural')
            st.plotly_chart(fig1)
        
        if column_total in merged_df.columns and column_urban in merged_df.columns:
        ...
        if 'State Name' in merged_df.columns:
            ...
            st.warning("State Name column not found.")

        # Plot
        st.subheader(f'Relationship between {worker_type} - Rural/Urban - Persons and {worker_type} - Total - Persons')

        # Create the plot
        if all(col in merged_df.columns for col in [f'{worker_type} - Rural -  Persons', f'{worker_type} - Urban -  Persons', f'{worker_type} - Total -  Persons']):
        ...
        merged_df = merged_df.dropna(subset=['State Name']) if 'State Name' in merged_df.columns else merged_df
        ...
        if geojson_data and 'State Name' in merged_df.columns:
            ...
            if column_name in merged_df.columns:
                ...
                st.warning(f"Column {column_name} not found for map visualization.")
            ...
            for state in merged_df['State Name'].unique():
                ...
                if not state_df.empty and 'NIC Name' in state_df.columns:
                    top_nic_name = state_df['NIC Name'].mode()
                    if not top_nic_name.empty:
                        st.write(f"Top NIC Name in {state}: {top_nic_name[0]}")
